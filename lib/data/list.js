    ...
    args.db.client.query(args.query, function (err, rows) {
        ...
        for (var i=0; i < rows.length; i++) {
            ...
            for (var j=idx; j < columns.length; j++) {
                ...
                values.push(value);
				if (columns[j].iFrameSource && args.config.table.iFrame) iFrameSource = `${args.config.table.iFrameSource}${value}`;
            }
			if (iFrameSource) {
				records.push({pk: pk, values: values, conversation: {source: iFrameSource}});
			} else {
				records.push({pk: pk, values: values});
			}
        };
        if (args.config.table.iFrame) {
            names.push(args.config.table.iFrameTitle)
        };
